...
class BsJPsiPhiGenAnalyzer_AOD(Analyzer):
    '''
    '''
    def declareHandles(self):
        super(BsJPsiPhiGenAnalyzer_AOD, self).declareHandles()
    ...
    def beginLoop(self, setup):
        super(BsJPsiPhiGenAnalyzer_AOD, self).beginLoop(setup)
        self.counters.addCounter('BsJPsiPhiGenAnalyzer_AOD')
        count = self.counters.counter('BsJPsiPhiGenAnalyzer_AOD')
    ...
    def process(self, event):
        ...
        self.counters.counter('BsJPsiPhiGenAnalyzer_AOD').inc('all events')
        ...
        #event.electrons = map(PhysicsObject, self.handles['electrons'].product())
        ...
        for ibs in event.gen_bsmesons:
            ...
            togenmatchhadrons = event.alltracks
            #print len(togenmatchhadrons)
            togenmatchhadrons = [ip for ip in togenmatchhadrons if ((ip.charge() != lep.charge() and abs(lep.bestTrack().eta() - ip.eta()) > 0.001 and abs(lep.bestTrack().phi() - ip.phi()) > 0.001 and abs(lep.bestTrack().pt() - ip.pt()) > 0.001) for lep in togenmatchleptons)]
            #print len(togenmatchhadrons)
            #isit, lp, lm, kp, km = self.isJPsiPhi(ibs, togenmatchleptons, event.alltracks, abs(self.cfg_ana.flavour)) 
            isit, lp, lm, kp, km = self.isJPsiPhi(ibs, togenmatchleptons, togenmatchhadrons, abs(self.cfg_ana.flavour)) 
            if isit:
                event.jpsiphi    = ibs
                event.jpsiphi.lp = lp
                event.jpsiphi.lm = lm
                ...
                if hasattr(event.jpsiphi.lp, 'reco') and hasattr(event.jpsiphi.lm, 'reco') and hasattr(event.jpsiphi.kp, 'reco') and hasattr(event.jpsiphi.km, 'reco'):
                    ...
                    self.tofit_pc.push_back(event.jpsiphi.kp.reco)
                    self.tofit_pc.push_back(event.jpsiphi.km.reco)
...
#                 import pdb ; pdb.set_trace()
        ...
        if not hasattr(event, 'jpsiphi'):
            return False
        self.counters.counter('BsJPsiPhiGenAnalyzer_AOD').inc('has a good gen Bs->JPsiPhi')
...
#             import pdb ; pdb.set_trace()
# 
#         self.counters.counter('BsJPsiPhiGenAnalyzer_AOD').inc('no fuck ups')
...
#         isAntiB0 = (not isB0)
    ...
    def isAncestor(a, p):
        ...
        for i in xrange(0,p.numberOfMothers()):
            if BsJPsiPhiGenAnalyzer_AOD.isAncestor(a,p.mother(i)):
                return True
        return False
