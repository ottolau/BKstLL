...
class BsJPsiPhiGenTreeProducer(L1PurityTreeProducerBase):
    ...
    def declareVariables(self, setup):
        ...
        self.var(self.tree, 'bs_lp_reco_idSelectorBit')
        self.var(self.tree, 'bs_lp_reco_idPatMVA')
        self.var(self.tree, 'bs_lp_reco_idPatSoftMVA')
        ...
        self.var(self.tree, 'bs_lm_reco_idSelectorBit')
        self.var(self.tree, 'bs_lm_reco_idPatMVA')
        self.var(self.tree, 'bs_lm_reco_idPatSoftMVA')
    ...
    def process(self, event):
        ...
        if hasattr(event, 'myB'):
            ...
            self.fill(self.tree, 'bs_lp_reco_idSelectorBit', event.myB.l0().selectors())
            self.fill(self.tree, 'bs_lp_reco_idPatMVA', event.myB.l0().mvaValue())
            self.fill(self.tree, 'bs_lp_reco_idPatSoftMVA', event.myB.l0().softMvaValue())
            self.fill(self.tree, 'bs_lp_reco_isLooseMuon', 1 if event.myB.l0().isLooseMuon() else 0)
            self.fill(self.tree, 'bs_lp_reco_isMediumMuon', 1 if event.myB.l0().isMediumMuon()  else 0)
            self.fill(self.tree, 'bs_lp_reco_isTightMuon', 1 if event.myB.l0().isTightMuon(recoVtx) else 0)
            ...
            self.fill(self.tree, 'bs_lm_reco_dxyError', event.myB.l0().muonBestTrack().dxyError())
            self.fill(self.tree, 'bs_lm_reco_dzError', event.myB.l0().muonBestTrack().dzError())
            ...
            self.fill(self.tree, 'bs_lm_reco_idSelectorBit', event.myB.l1().selectors())
            self.fill(self.tree, 'bs_lm_reco_idPatMVA', event.myB.l1().mvaValue())
            self.fill(self.tree, 'bs_lm_reco_idPatSoftMVA', event.myB.l1().softMvaValue())
            self.fill(self.tree, 'bs_lm_reco_isLooseMuon', 1 if event.myB.l1().isLooseMuon() else 0)
            self.fill(self.tree, 'bs_lm_reco_isMediumMuon', 1 if event.myB.l1().isMediumMuon()  else 0)
            self.fill(self.tree, 'bs_lm_reco_isTightMuon', 1 if event.myB.l1().isTightMuon(recoVtx) else 0)

            self.fill(self.tree, 'bs_kk_reco_dR', event.myB.phicone())
            self.fill(self.tree, 'bs_kp_reco_dxy', event.myB.kp().bestTrack().dxy(event.myB.bs()))
            self.fill(self.tree, 'bs_kp_reco_dz', event.myB.kp().bestTrack().dz(pv))
            self.fill(self.tree, 'bs_kp_reco_dxyError', event.myB.kp().bestTrack().dxyError())
            self.fill(self.tree, 'bs_kp_reco_dzError', event.myB.kp().bestTrack().dzError())
            self.fill(self.tree, 'bs_kp_reco_chi2', event.myB.kp().bestTrack().chi2())
            self.fill(self.tree, 'bs_kp_reco_ndof', event.myB.kp().bestTrack().ndof())
            self.fill(self.tree, 'bs_km_reco_dxy', event.myB.km().bestTrack().dxy(event.myB.bs()))
            self.fill(self.tree, 'bs_km_reco_dz', event.myB.km().bestTrack().dz(pv))
            self.fill(self.tree, 'bs_km_reco_dxyError', event.myB.km().bestTrack().dxyError())
            self.fill(self.tree, 'bs_km_reco_dzError', event.myB.km().bestTrack().dzError())
            self.fill(self.tree, 'bs_km_reco_chi2', event.myB.km().bestTrack().chi2())
            self.fill(self.tree, 'bs_km_reco_ndof', event.myB.km().bestTrack().ndof())
        ...
        if len(event.clean_gen_bmesons)>0:
            ...
            self.fill(self.tree, 'dphi_btag_bprobe' , deltaPhi(event.clean_gen_bmesons[0].phi(), event.jpsiphi.phi()) )
        if hasattr(event, 'thetagmu'):
            self.fill(self.tree, 'dr_mutag_bprobe'  , deltaR  (event.thetagmu, event.jpsiphi)                         )
            self.fill(self.tree, 'deta_mutag_bprobe', abs     (event.thetagmu.eta() - event.jpsiphi.eta())            )
            self.fill(self.tree, 'dphi_mutag_bprobe', deltaPhi(event.thetagmu.phi(), event.jpsiphi.phi())             )
