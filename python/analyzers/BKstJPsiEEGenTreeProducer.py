...
class BKstJPsiEEGenTreeProducer(L1PurityTreeProducerBase):
    ...
    def process(self, event):
        ...
        if hasattr(event, 'myB'):
            ...
            self.fill(self.tree, 'bs_lp_reco_dxy', event.myB.l0().dxy(event.myB.bs()))
            self.fill(self.tree, 'bs_lp_reco_dz', event.myB.l0().dz(pv))
            self.fill(self.tree, 'bs_lp_reco_dxyError', event.myB.l0().dxyError())
            self.fill(self.tree, 'bs_lp_reco_dzError', event.myB.l0().dzError())
            self.fill(self.tree, 'bs_lp_reco_chi2', event.myB.l0().chi2())
            self.fill(self.tree, 'bs_lp_reco_ndof', event.myB.l0().ndof())

            self.fill(self.tree, 'bs_lm_reco_dxy', event.myB.l1().dxy(event.myB.bs()))
            self.fill(self.tree, 'bs_lm_reco_dz', event.myB.l1().dz(pv))
            self.fill(self.tree, 'bs_lm_reco_dxyError', event.myB.l1().dxyError())
            self.fill(self.tree, 'bs_lm_reco_dzError', event.myB.l1().dzError())
            self.fill(self.tree, 'bs_lm_reco_chi2', event.myB.l1().chi2())
            self.fill(self.tree, 'bs_lm_reco_ndof', event.myB.l1().ndof())
