...
class L1PurityTreeProducer(L1PurityTreeProducerBase):
    ...
    def process(self, event):
        ...
        fired, matched = single_muon(event.L1_muons, 10, 1.5,  8, matches=event.gen_bmesons); self.fill(self.tree, 'L1_SingleMu_15_eta1p5_Q8' , fired) ; self.fill(self.tree, 'matched_L1_SingleMu_15_eta1p5_Q8' , matched)
        fired, matched = single_muon(event.L1_muons, 10, 1.5, 12, matches=event.gen_bmesons); self.fill(self.tree, 'L1_SingleMu_15_eta1p5_Q12', fired) ; self.fill(self.tree, 'matched_L1_SingleMu_15_eta1p5_Q12', matched)
